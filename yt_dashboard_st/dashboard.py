# -*- coding: utf-8 -*-
"""
Created on Mon Jun 24 15:14:06 2024
...
@st.cache_data
def load_data():
    df_agg = pd.read_csv('yt_dashboard_st/Aggregated_Metrics_By_Video.csv').iloc[1:,:]
    ...
    df_agg_sub = pd.read_csv('yt_dashboard_st/Aggregated_Metrics_By_Country_And_Subscriber_Status.csv')
    df_comments = pd.read_csv('yt_dashboard_st/All_Comments_Final.csv')
    df_time = pd.read_csv('yt_dashboard_st/Video_Performance_Over_Time.csv')
...
#ADDITIONAL DATA ENGINNERING
df_agg_diff = df_agg.copy()
metric_date_12mo = df_agg_diff['Video publish time'].max() - pd.DateOffset(months =24)
...
date_12mo = df_agg['Video publish time'].max() - pd.DateOffset(months =24)
...
if add_sidebar == 'Aggregate Metrics':
    ...
    metric_date_6mo = df_agg_metrics['Video publish time'].max() - pd.DateOffset(months =18)
    metric_date_12mo = df_agg_metrics['Video publish time'].max() - pd.DateOffset(months =24)
    metric_medians6mo = df_agg_metrics[df_agg_metrics['Video publish time'] >= metric_date_6mo].iloc[:, 1:].median()
    metric_medians12mo = df_agg_metrics[df_agg_metrics['Video publish time'] >= metric_date_12mo].iloc[:, 1:].median()
    ...
    df_agg_numeric_lst = df_agg_diff_final.iloc[:, 2:].median().index.tolist()
    df_to_pct = {}
...
if add_sidebar == 'Individual Video Analysis':
    ...
    fig2.add_trace(go.Scatter(x=views_cumulative['days_published'], y=views_cumulative['20pct_views'],
                    mode='lines',
                    name='20th percentile', line=dict(color='violet', dash ='dash')))
